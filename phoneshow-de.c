...
static void show_help (void)
{
  ...
  "Exitcode       %i wenn mindestens ein phonetisch ähnlicher Name gefunden wurde\n"
  "               %i wenn kein phonetisch ähnlicher Name gefunden wurde\n"
  "               %i wenn Name zu kurz zum sinnvollen codieren\n"
  "               %i wenn Name zu lang\n"
  "               %i wenn auf Grund des Namens keine kodierung erfolgt z.B. phoneshow-de -p Aahe\n"
  ...
  "               %i wenn von stdin Textzeilen oder Worte mit Überlänge empfangen wurden\n"
  "               %i wenn es bei Speicheranfordeung zu Problemen kam\n\n", STAT_FOUND, STAT_NO_FOUND, STAT_TO_SHORT,
...
{ 
  ...
  if ((Cur_Line_Text[readpos] >= 'a' && Cur_Line_Text[readpos] <= 'z') ||
    (Cur_Line_Text[readpos] >= 'A' && Cur_Line_Text[readpos] <= 'Z')) {
      word_mode = true;
  }
  else {
    for (cnt=0; cnt<=6; cnt++) {
  ...
  // War Sonderzeichen, dieses in Cur_Word schieben und raus
  if (word_mode == false) {
...
static bool check_names_list_phone_equal (int *name_number)
{
  ...
  // Natürlich mit false, da das Wort ja ausgelassen werden soll
  for (cnt = 0; cnt < Number_Of_Names; cnt++) {
...
static void show_footer (const size_t foundcount)
{
  int cnt;
  int nolist=0;
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (word_stat == true) {                                                  // War gültiges deutsche Wort
        ...
        if (phone_stat == true) {                                               // Wir haben einen Fund
          foundcount++;
          if (Phone_Ops.n == true) printf("%zd ", Cur_Line_Number);
          if (Phone_Ops.f == true) {
...
int main (int argc, char* argv[])
{
  ...
  outops_t  outops = {false, false, false}; // Ausgabeformat
