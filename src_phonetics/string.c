...
// Rückgabe:  true  bei Fund, Startposition der Gruppe in "start", Länge der Gruppe in "len"
...
int str_to_word (const char *src, word_t dest)
{
  ...
  if (strlen(src) >= BUFFSIZE_WORD) {
    dest[0] = '\0';
    return STR_TO_WORD_ERR_OVERLEN;
  }
  else strcpy(dest, src);
  
  // Leerstrings sind keine Worte
  if (dest[0] == '\0') return STR_TO_WORD_ERR_NO_WORD;

NextChar:
  if (dest[pos] == '\0') return STR_TO_WORD_SUCCESS;
  ...
  dest[0]='\0';
  return STR_TO_WORD_ERR_NO_WORD;
}
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
